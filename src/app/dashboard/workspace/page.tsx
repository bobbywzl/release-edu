...
function WorkspaceInner() {
  ...
  // Autosave state: notes are the learner's own words and must never be lost
  // to a closed tab. 'saving' → 'saved <time>' is shown live under the box.
  ...
  async function streamFromBob(text: string, showUser: boolean, atts: ChatAttachment[] = []) {
    // streamingRef is the synchronous guard: two Enter events can land in the
    // same frame, both reading a stale `streaming` closure — the ref flips
  ...
  // AUTOSAVE: debounce 1.2s after typing stops, and flush on unmount / node
  // switch — a learner who closes the tab mid-thought keeps their words.
  useEffect(() => {
    if (notesDraft === null || !treeId || !nodeId) return
    if (notesTimer.current) clearTimeout(notesTimer.current)
    notesTimer.current = setTimeout(() => { void saveNotes() }, 1200)
    return () => { if (notesTimer.current) clearTimeout(notesTimer.current) }
    // saveNotes closes over the current draft/node; re-running on draft change
    // is exactly the debounce we want.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [notesDraft, treeId, nodeId])
          ...
          (() => {
            ...
            return (
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {/* "Quiz me" moved INTO the chat: it appears as an option
                    when Bob invites ("ready for a challenge?" + [[QUIZ_OFFER]])
                    rather than living up here where clicking it yanked the
                    learner away from the conversation. */}
              </div>
            )
          })()
        )}
                  ...
                  <div className={cn(
                    ...
                    {m.role === 'user' ? m.content : (
                      <>
                        ...
                        {/* Bob's readiness invitation — the conversational
                            "Quiz me" option. Only on the latest turn, only
                            while no checkpoint card is already live. */}
                        {m.content.includes('[[QUIZ_OFFER]]') && mi === messages.length - 1 && !streaming && !activeQuiz && node?.status !== 'understood' && (
                          <div className="mt-2.5 border-t border-border/60 pt-2.5">
                            <button
                              onClick={() => streamFromBob(t('workspace.quizMeMessage'), true)}
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-emerald-400/40 bg-emerald-500/10 text-emerald-300 text-xs font-semibold hover:bg-emerald-500/20 transition-colors"
                            >
