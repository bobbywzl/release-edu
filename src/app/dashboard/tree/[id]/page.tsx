...
import {
  ...
  Bot, Paperclip, Camera, Video, Mic, Square, Send,
} from 'lucide-react'
...
function TreeCanvasInner() {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Tree Copilot — one chatbox for every tree-level function */}
      <TreeCopilot
        tree={tree}
        onChanged={load}
...
/**
 * TREE COPILOT — the tree page's single conversational box combining every
 ...
 * (approval chip), all with SOTA-chatbot multimodal input (files, camera
 * photo/video on mobile via capture inputs, voice via MediaRecorder) and
 * generated visuals (Bob's ```image blocks render through MarkdownRenderer).
 */
function TreeCopilot({ tree, onChanged, fit }: {
  tree: TreeData
  ...
  // This dialog's unapproved ghosts — replaced by the next turn's proposals.
  ...
  useEffect(() => { if (open) endRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [thread, open])
  ...
  // rejected with a visible note, never silently truncated (truncated base64
  // is corrupt).
  const MAX_ATTACH = 3_000_000

  async function addFile(files: FileList | null, fallbackName: string) {
    const file = files?.[0]
    if (!file) return
    if (file.size > MAX_ATTACH) { setNote(t('tree.copilotTooLarge')); return }
  ...
  async function toggleRecord() {
    if (recording) { try { recRef.current?.stop() } catch { /* already stopped */ } return }
    try {
      ...
      rec.onstop = () => {
        ...
        if (blob.size > MAX_ATTACH) { setNote(t('tree.copilotTooLarge')); return }
        const r = new FileReader()
  ...
  async function send() {
    ...
    const label = attachments.length ? `${msg}${msg ? '\n' : ''}[${attachments.map(a => `📎 ${a.name}`).join(' · ')}]` : msg
    setThread(t2 => [...t2, { role: 'user', content: label }])
    ...
    try {
      const res = await fetch(`/api/tree/${tree.id}/copilot`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: msg, lang: language, replaceIds: ghostIds, attachments: payload }),
      })
      const body = await res.json().catch(() => ({}))
      if (!res.ok) {
      ...
      if (proposals.length > 0) {
        setGhostIds(proposals.map((p: { id?: string }) => p?.id).filter(Boolean) as string[])
  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-5 right-5 z-40 inline-flex items-center gap-2 rounded-full bg-primary text-primary-foreground pl-4 pr-5 py-3 text-sm font-semibold shadow-xl hover:bg-primary/90 transition-colors"
        title={t('tree.copilotHint')}
      >
        <Bot className="w-5 h-5" /> {t('tree.copilotTitle')}
      </button>
    )
  }

  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col w-[min(27rem,calc(100vw-2rem))] h-[min(36rem,78vh)] bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-2.5 px-4 py-3 border-b border-border bg-background/50">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground truncate">{tree.title}</p>
        </div>
        <button onClick={() => setOpen(false)} className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent transition-colors" aria-label={t('common.dismiss')}>
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {thread.length === 0 && (
          <p className="text-xs text-muted-foreground leading-relaxed px-1 py-2">{t('tree.copilotHint')}</p>
        )}
        {thread.map((m, i) => (
          <div key={i} className={cn('flex', m.role === 'user' ? 'justify-end' : 'justify-start')}>
            <div className={cn(
              'rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed max-w-[92%]',
              m.role === 'user'
        ...
        {purposeProposal && (
          <div className="border border-emerald-400/40 bg-emerald-500/[0.08] rounded-xl p-3 space-y-2">
            <p className="text-[11px] font-bold text-emerald-300 uppercase tracking-wider">{t('tree.copilotPurposeTitle')}</p>
            <p className="text-xs text-foreground leading-relaxed">{purposeProposal}</p>
            <div className="flex gap-2">
              <button
            ...
            <div className="flex flex-wrap gap-1.5">
              {attachments.map((a, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 text-[11px] bg-background border border-border rounded-full pl-2.5 pr-1.5 py-1 text-foreground/85 max-w-[240px]">
                  ...
                  <button onClick={() => setAttachments(x => x.filter((_, j) => j !== i))} className="p-0.5 rounded-full hover:bg-accent text-muted-foreground hover:text-foreground">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
      ...
      {/* Input row — SOTA-chatbot multimodal controls */}
      <div className="border-t border-border p-2.5 space-y-1.5">
        <div className="flex items-center gap-1">
          <input ref={fileRef} type="file" className="hidden" onChange={e => { addFile(e.target.files, 'file'); e.target.value = '' }} />
          <input ref={photoRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={e => { addFile(e.target.files, 'photo.jpg'); e.target.value = '' }} />
          <input ref={videoRef} type="file" accept="video/*" capture="environment" className="hidden" onChange={e => { addFile(e.target.files, 'video.mp4'); e.target.value = '' }} />
          <button onClick={() => fileRef.current?.click()} title={t('tree.copilotAttachFile')} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
            <Paperclip className="w-4 h-4" />
          </button>
          <button onClick={() => photoRef.current?.click()} title={t('tree.copilotAttachPhoto')} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
            <Camera className="w-4 h-4" />
          </button>
          <button onClick={() => videoRef.current?.click()} title={t('tree.copilotAttachVideo')} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors">
        ...
        <div className="flex gap-1.5">
          <textarea
            ...
            className="flex-1 bg-background border border-border rounded-xl px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground resize-none focus:outline-none focus:ring-1 focus:ring-primary/40"
          />
          <button
            onClick={send}
            disabled={busy || (!input.trim() && attachments.length === 0)}
            className="self-end inline-flex items-center justify-center rounded-xl bg-primary text-primary-foreground p-2.5 hover:bg-primary/90 transition-colors disabled:opacity-40"
          >
