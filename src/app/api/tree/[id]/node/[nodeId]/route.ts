 ...
 *   { action: 'edit', title?, summary? }     — rewrite title/summary (edit mode / rename)
 ...
 *   + split / merge / spinoff / rebalance / reorder (structural ops)
 *
 * Structural op implementations live in src/lib/tree-ops.ts (shared with the
 * copilot's atomic rewire plan). Destructive ops snapshot the tree first —
 * the undo pill on the tree page restores it.
 *
...
import {
  TreeOpError, snapshotTree, discardSnapshot,
  opEdit, opMove, opDelete, opAddChild, opMerge, opReorder, opSplit, opRebalance, opSpinoff,
} from '@/lib/tree-ops'
...
export async function PATCH(
  ...
  // Destructive structural ops run through the shared executors with a
  // snapshot taken FIRST. A TreeOpError means the op validated-and-refused
  // BEFORE writing — only then is the unused snapshot discarded. Any other
  // failure may have landed partial writes, so the snapshot must survive as
  // the recovery path (the undo pill).
  const structural = async (label: string | null, run: () => Promise<Record<string, unknown> | void>) => {
    const snapId = label ? await snapshotTree(userId, id, label) : null
    try {
  ...
  switch (body.action) {
    ...
    case 'split':
      return structural('split', async () => {
        const created = await opSplit(userId, id, nodeId, {
          title: body.title ?? '', summary: body.summary, moveTail: body.moveTail,
        })
        return { node: created }
      })
    case 'merge':
      return structural('merge', () => opMerge(userId, id, nodeId, body.intoNodeId ?? ''))
    case 'spinoff':
      // No snapshot: nothing is destroyed — the subtree lives on in its own
      // tree, and a restore here would collide with those moved node ids.
      return structural(null, async () => {
        const { newTreeId } = await opSpinoff(userId, id, nodeId)
        await normalizeTreeKinds(newTreeId)
        return { newTreeId }
      })
    case 'rebalance':
      return structural('rebalance', async () => {
        const created = await opRebalance(id, nodeId, {
          title: body.title ?? '', summary: body.summary, facets: Array.isArray(body.facets) ? body.facets : [],
        })
        return { node: created }
      })
    case 'reorder':
      return structural(null, () => opReorder(id, nodeId, Array.isArray(body.childIds) ? body.childIds : []))
    case 'reject': {
    ...
    case 'add_child':
      return structural(null, async () => {
        const created = await opAddChild(id, nodeId, {
          title: body.title ?? '', summary: body.summary, origin: 'manual',
        })
        return { node: created }
      })
    case 'edit':
      return structural('edit', () => opEdit(id, nodeId, { title: body.title, summary: body.summary }))
    case 'move':
      return structural('move', () => opMove(id, nodeId, body.newParentId ?? ''))
    case 'delete':
      return structural('delete', async () => {
        const deleted = await opDelete(id, nodeId)
        return { deleted }
      })
    case 'notes': {
