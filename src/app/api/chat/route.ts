...
async function extractInsightsBackground(
  ...
  try {
    ...
    const result = await client.messages.create({
      // Background insight extractor — emits {type, content, confidence}
      // tuples. Classification with calibrated confidence scoring, well
      // within Haiku capability. The "confidence/gap" metrics the user
      // called out specifically.
      model: pickBackgroundModel(),
      max_tokens: 500,
      messages: [{
        role: 'user',
        content: `Analyze this student-tutor exchange and extract insights about the student. Only extract CLEAR signals - don't guess.

Student said: "${userMessage.slice(0, 500)}"
Tutor responded: "${assistantResponse.slice(0, 500)}"

For each insight, provide:
- type: one of personality | interest | strength | weakness | preference | aspiration | breakthrough | struggle | style
- content: one sentence describing what you observed
- confidence: 0.0 to 1.0

Return ONLY a JSON array. If nothing notable, return [].
Example: [{"type":"interest","content":"Shows growing curiosity about machine learning","confidence":0.7}]`,
      }],
    })
    ...
    const parsed = JSON.parse(text) as { type: string; content: string; confidence: number }[]
    if (!Array.isArray(parsed)) return

    const store = dbStore.forUser(storeUserId)
    for (const insight of parsed) {
      if (insight.type && insight.content && insight.confidence) {
