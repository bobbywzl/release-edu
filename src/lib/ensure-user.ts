 ...
 * StudentProfile, UserBadge…) FKs onto User, but first-action users
 * historically had no row, so their first write 502'd AFTER the AI call was
 * already paid for (simulation finding: two billed tree-seeds, zero trees).
 */
...
export async function ensureUserRow(userId: string): Promise<void> {
  ...
  let email = `placeholder-${userId}@pending.edu`
